...
export default function EditCV({ params }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/cv/${id}`)
      ...
      .catch(() => { toast.error('Failed to load CV'); router.push('/admin/cv') })
  }, [id, router])
  ...
  // Skills
  function addSkill() { setData('skills', [...(form?.skills || []), { name: '', level: 80 }]) }
  function updateSkill(i, key, val) {
    const s = [...form.skills]; s[i] = { ...s[i], [key]: val }; setData('skills', s)
  }
  function removeSkill(i) { setData('skills', form.skills.filter((_, idx) => idx !== i)) }

  // Experience
  function addExp() { setData('experience', [...(form?.experience || []), { position: '', company: '', period: '', desc: '' }]) }
  function updateExp(i, key, val) {
    const s = [...form.experience]; s[i] = { ...s[i], [key]: val }; setData('experience', s)
  }
  function removeExp(i) { setData('experience', form.experience.filter((_, idx) => idx !== i)) }

  // Education
  function addEdu() { setData('education', [...(form?.education || []), { school: '', degree: '', year: '' }]) }
  function updateEdu(i, key, val) {
    const s = [...form.education]; s[i] = { ...s[i], [key]: val }; setData('education', s)
  }
  function removeEdu(i) { setData('education', form.education.filter((_, idx) => idx !== i)) }
    ...
    <div className="max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input id="title" value={form.title} onChange={(e) => setData('title', e.target.value)} required />
            </div>
            <div className="space-y-2">
        ...
        <section className="space-y-3">
          ...
          {(!form.skills || form.skills.length === 0) && <p className="text-gray-400 text-sm">Belum ada skill.</p>}
        </section>
        ...
        <section className="space-y-3">
          ...
          {(!form.experience || form.experience.length === 0) && <p className="text-gray-400 text-sm">Belum ada pengalaman.</p>}
        </section>
        ...
        <section className="space-y-3">
          ...
          {(!form.education || form.education.length === 0) && <p className="text-gray-400 text-sm">Belum ada pendidikan.</p>}
        </section>
